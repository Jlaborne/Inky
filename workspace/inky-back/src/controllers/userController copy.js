...
const registerUser = async (req, res) => {
  ...
  try {
    ...
    // 2) DB (sans password)
    const user = new User(createdFirebaseUid, lastName, firstName, email, role);
    ...
    // Si DB a échoué après création Firebase → rollback Firebase
    if (createdFirebaseUid) {
      try {
        await admin.auth().deleteUser(createdFirebaseUid);
      } catch (rollbackErr) {
        // on log uniquement, on garde 500 pour la réponse
        console.error("Rollback Firebase user failed:", rollbackErr);
      }
    }
